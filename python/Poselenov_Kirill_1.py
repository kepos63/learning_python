...
def registration():
    ...
    note  = cursor.execute("""SELECT * FROM user WHERE login = ?""", [login])
    if  note.fetchone() is None and len(password) >= 8 and (login != '' or password != ''): # Проверка на существования пользователя и длинна пароля равна или более 8 символов
...
lbl_name_dev = Label(root, text='Разработчик: Поселенов Кирилл Евгеньевич',bg = '#4D4D5E', fg='white', font='Arial 15' )# Текстовый виджет (аналог print)
...
ent_login = Entry(root, bg = 'white', fg = 'black', font='Consolas 14', justify='center', width=40) # Поле для ввода значения
...
btn_reg = Button(root, text = 'Регистрация', bg = '#D6D6E0', fg = 'Black', font = 'Consolas 14', command = registration)
